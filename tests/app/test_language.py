...
def test_switching_language_updates_open_windows_and_the_tray(
    ...
    try:
        assert note.title_bar.close_button.toolTip() == "Close note"
        assert tray.new_note_action.text() == "New note"

        translations.apply("ko")

        assert note.title_bar.close_button.toolTip() == "메모 닫기"
        ...
        assert manager.new_note().title_bar.close_button.toolTip() == "메모 닫기"
